...
class Aschroder_SMTPPro_Model_Mysql4_Email_Queue extends Mage_Core_Model_Resource_Email_Queue
{
    /**
    ...
    public function wasEmailQueued(Mage_Core_Model_Email_Queue $queue)
    {
        ...
        if ($queue->hasAttachments()) {
            $hashs = array();
            foreach ($queue->getAttachments() as $attachment) {
                $hashs[] = $attachment->getBodyHash();
            }
            $cond = $readAdapter->prepareSqlCondition('attachments.body_hash', array('in' => $hashs));
            $select->join(
                array('attachments' => $this->getTable('smtppro/email_attachment')),
                "queue.message_id = attachments.message_id and $cond",
                array()
            );
        }
